...
const AppMenu = ({ menuItems }: AppMenuProps): React$Element<React$FragmentType> => {
    ...
    const [menuItemsArr, setMenuItemsArr] = useState(menuItems)
    ...
    const toggleMenu = (menuItem, show) => {
        if (show) setActiveMenuItems([menuItem['key'], ...findAllParent(menuItemsArr, menuItem)]);
    };

    /**
     * activate the menuItemsArr
     */
    const activeMenu = useCallback(() => {
        ...
        if (div) {
            ...
            if (matchingMenuItem) {
                ...
                const activeMt = findMenuItem(menuItemsArr, mid);
                if (activeMt) {
                    setActiveMenuItems([activeMt['key'], ...findAllParent(menuItemsArr, activeMt)]);
                }
            }
        }
    }, [location.pathname, menuItemsArr]);

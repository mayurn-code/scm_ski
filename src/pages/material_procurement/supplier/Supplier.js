...
import { master_list, SupplierList } from '../../../helpers/api'
...
const ActionColumn = ({ row }) => {
    return (
        <>
            <Link to={`/material-procurement/supplier/${row.original["data-supplier_Id"]}`} className="action-icon">
                {' '}
...
const SupplierListComp = ({ selectedSupplierArr, materialid,
    ...
    const loadTrasporter = (materialid, pickupcityid, dropcityid) => {
        ...
        SupplierList(moduleName, materialid, pickupcityid, dropcityid).then(res => {
            if (res !== undefined) {
    ...
    const handelAddRFQBtn = () => {
        ...
        if (markedCheckbox.length === 0) {
            ...
            if (listtype === "createrfq") {
                navigate('/material-procurement/add-rfq', {
                    state: obj
                })
            }
            if (listtype === "directorder") {
                navigate('/material-procurement/add-po', {
                    state: obj
                })
            }
        }
